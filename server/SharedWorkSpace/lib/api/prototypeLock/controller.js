...
module.exports.updatePrototypeLock = function (req, res) {
    serviceLogger.info({params: req.params}, '>> keepliveUpdate()');

    prototype.updateTimeInLock(_getProjectId(req)).then(
        function (data) {
            res.status(200).json(data);
        }, function (error) {
            serviceLogger.error({params: error}, '>> PrototypeService.updatePrototypeLock()');
            res.status(500).json(error);
