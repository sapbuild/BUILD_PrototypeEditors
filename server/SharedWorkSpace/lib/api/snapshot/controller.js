        ...
        if (prototype !== undefined) {
            provideSnapshotVersionForZipContent(projectId, snapshotVersion)
                .then(function (version) {
                    if (version) {
                        getAllFilesForSnapshot(version)
...
var deleteFolderRecursive = function (path) {
    ...
    if (fs.existsSync(path)) {
        ...
        fs.rmdirSync(path, function (error) {
            deferred.reject(new NormanError(commonMessage.error.SWE011, 'SWE011', error));
        });

        deferred.resolve(null);
    }
    else {
        deferred.reject(new NormanError(commonMessage.error.SWE011, 'SWE011'));
    }
