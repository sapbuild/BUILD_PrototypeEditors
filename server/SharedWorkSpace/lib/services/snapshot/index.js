    ...
    updateSnapshotVersion(projectId, snapshotDesc, createdBy)
        .then(function (snaprecord) {
            ...
            ArtifactService.getArtifactByMetadata({'metadata.projectId': projectId}).then(function (artifactToSnapshot) {
                ...
                var artifactWithoutSnapshot = _.filter(artifactToSnapshot, function (artifact) {
                    return (!artifact.metadata.hasOwnProperty('snapshotVersion'));
                });

                artifactToSnapshot = _.uniq(artifactWithoutSnapshot, function (artifact) {
                ...
                Promise.all(promises).then(function (copiedArtifacts) {
                    var PrototypeService = registry.getModule('PrototypeService');
                    PrototypeService.getMetadata(projectId, ['appMetadata', 'pageMetadata'])
                        .then(function (response) {
                            ...
                            copiedArtifacts.forEach(function (copiedArtifact) {
                                response.artifacts.forEach(function (artifact) {
                                    if (artifact.path === copiedArtifact.metadata.path) {
                                        artifactsIds.push(copiedArtifact._id.toString());
                                    }
                                });
                            });
                    ...
                    .then(function (appMetadata) {
                        ...
                        appMetadata.pages.forEach(function (page) {
                            ...
                            if (!appMetadata.isSmartApp) {
                                ...
                                snapshotPage = _.find(snapshotConfiguration, function (configurationPage) {
                                ...
                                if (firstPage) {
                                    ...
                                    snapshotPage = _.find(snapshotConfiguration.pages[0].pages, function (configurationPage) {
                                        return configurationPage.pageName === page.name;
                                    });
                                    if (snapshotPage) {
                                        pageUrl += '#/' + snapshotPage.entitySet + '(' + snapshotPage.defaultContext + ')';
                            ...
                            deepLinks.push({pageName: pageName, thumbnail: thumbnail, pageUrl: pageUrl, displayName: displayName});
                        });
