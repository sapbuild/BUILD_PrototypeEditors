...
var R7b = 'Invalid column name for the target Object. Please check the syntax: To.[Object name]';
...
function getCardinalityForMappingTable(table, messages) {
    var target = table.columns[1] && table.columns[1].name ? table.columns[1].name.split(SEPARATOR)[1] : null,
        cardinality = false,
        keys,
        inverseKeys,
        unique;
...
function fillRelationTable(table, messages) {
    var split = table.name.split(SEPARATOR),
        source = split[0],
        relationName = split[1],
        target = table.columns[1] && table.columns[1].name ? table.columns[1].name.split(SEPARATOR)[1] : null,
        data = [];
...
function generateRelationFromFK(table, property, entities) {
    var split = property.name.split(SEPARATOR),
        source = split[0],
        relationName = split[1],
        target = split[2],
        data = [];
...
function createModel(tables) {
    ...
    tables.forEach(function (table) {
        if (table.name.indexOf(SEPARATOR) < 0) {
            var entity = {
                name: table.name,
    ...
    tables.forEach(function (table) {
        ...
        table.columns.forEach(function (column) {
            ...
            if (property.name.indexOf(SEPARATOR) !== -1) {
...
function checkName(name, messages, table, cell) {
    var ln = messages.length;

    if (!ODATA_IDENTIFIER.test(name)) {
        if (name.length > 40) {
            messages.push(createError(R9a, 'R9a', table));
        }
        else if (!/[a-z_]/i.test(name.substr(0, 1))) {
...
function checkTables(tables, messages, existingModel) {
    ...
    tables.forEach(function (table) {
        if (table.name.indexOf(SEPARATOR) === -1) {
        ...
        tables.some(function (table) {
            if (table.name.indexOf(SEPARATOR) === -1) {
                table.columns.some(function (column) {
                    hasForeignKeys = (column.name.indexOf(SEPARATOR) > 0);
    ...
    tables.forEach(function (table) {
        ...
        if (table.name.indexOf(SEPARATOR) > 0) {
            var s = table.name.split(SEPARATOR);
            ...
            if (!checkName(s[1], messages, table)) {
                return false;
            }
            ...
            if (!checkName(table.name, messages, table)) {
                return false;
            }
            ...
            table.columns.forEach(function (column, index) {
                var cell = _getCellRef(table.origin, index, 0),
                    els = column.name.split(SEPARATOR);
                ...
                // if this is a valid foreign key, check existence of the involved objects
                    ...
                    if (oRelations[els[0] + SEPARATOR + els[1]]) {
                        messages.push(createError(R10a.replace('{Object name}', els[0] + SEPARATOR + els[1]), 'R10a', table, cell));
                        return false;
                    }

                    oRelations[els[0] + SEPARATOR + els[1]] = true;
                }
                // not a valid foreign key
                else if (els.length === 3) {
                    lodash.forEach(els, function (element) {
                        if (!checkName(element, messages, table)) {
                            return false;
                        }
                    });
                }
                ...
                else if (!checkName(column.name, messages, table, cell)) {
                    return false;
                }
...
function mergeData(entityTable, mappingTable, pkIdColumn, fkIdColumn, idPropName, pkPropertyName, messages) {
    lodash.forEach(entityTable.data, function (line) {
        line[pkPropertyName] = null;
    });
...
function checkRelationForUpdate(messages, elements, mappingTables) {
    // serveral entity: we check if there are all relations
    if (Object.keys(elements).length > 1) {
        lodash.forEach(elements, function (element) {

            if (mappingTables.length !== 0) {
                // Import with mapping table
                lodash.forEach(element.entity.navigationProperties, function (navProp) {
                    if (lodash.findIndex(mappingTables, {name: element.entity.name + SEPARATOR + navProp.name}) === -1) {
                        messages.push(createError(R12.replace('{name}', element.table.name), 'R12', element.table));

                    }
                });
            }
            else {
                // Import whitout mapping table
                lodash.forEach(element.entity.navigationProperties, function (navProp) {
                    var table, toElement;
                    lodash.forEach(elements, function (element) {
                        if (element.entity._id === navProp.toEntityId) {
                            toElement = element;
                            return false;
                        }
                    });
                    ...
                    if (toElement) {
                        table = navProp.multiplicity ? toElement.table : element.table;
                    }
                    else {
                        messages.push(createError(R12.replace('{name}', element.table.name), 'R12', element.table));
                    }
                    ...
                    if (table && toElement
                        && lodash.findIndex(table.columns, {name: element.entity.name + SEPARATOR + navProp.name + SEPARATOR + toElement.entity.name}) === -1) {
                        messages.push(createError(R12.replace('{name}', element.table.name), 'R12', element.table));
                    }
                });
            }
...
exports.getData = function (context) {
    ...
    try {
        ...
        if (tables.length > 0) {
            ...
            checkRelationForUpdate(messages, elements, mappingTables);
            ...
            if (mappingTables.length > 0) {
                ...
                mappingTables.forEach(function (mappingTable) {
                    ...
                    split = mappingTable.name.split(SEPARATOR);
            ...
            Object.keys(elements).forEach(function (key) {
                ...
                element.table.columns.forEach(function (column, index) {
                    ...
                    split = column.name.split(SEPARATOR);
