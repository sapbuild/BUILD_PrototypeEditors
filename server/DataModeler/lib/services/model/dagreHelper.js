...
exports.addEntityPosition = function (context) {
    var id = context.newEntity._id;
    var newEntity, topEntity;
    lodash.each(context.model.entities, function (entity) {
        if (entity._id === id) {
            newEntity = entity;
        }
        else if (!topEntity || topEntity.position.top < entity.position.top) {
            topEntity = entity;
        }
    });
    ...
    if (newEntity) {
        ...
        if (topEntity) {
            newEntity.position = {
                left: topEntity.position.left,
                top: topEntity.position.top + NODE_SEPARATION + NODE_HEIGHT,
                width: NODE_WIDTH,
                height: NODE_HEIGHT
            };
        }
