...
exports.incrementData = function (context) {
	...
	if (context.entityData) {
        ...
        oResult = Promise.resolve(context);
	}
	return oResult;
};
			...
			data.forEach(function (oNewDataRow) {
				...
				if (oOldRow) {
					for (var sPropertyName in oOldRow) {
						...
						if (navigationPropertyHelper.isForeignKey(sPropertyName)) {
							if (oNewDataRow[sPropertyName] === undefined) {
								oNewDataRow[sPropertyName] = oOldRow[sPropertyName];

							}
							else if (oNewDataRow[sPropertyName] === null) {
								oNewDataRow[sPropertyName] = undefined;
