...
exports.extractAnnotationFromPageMetadata = function (pageMetadata, assetsToCopy) {
    ...
    function _extractAnnotationFromControl(controlId, designTemplateElement, sourceVariableMap) {
        ...
        if (controlData && builderUtils.isControlValid(controlData)) {
            // For ease of parsing we start by sorting properties by their annotationDepth and start by the highest level annotations
                    ...
                    if (property.binding != null) {
                        ...
                        if (bindingInfo && bindingInfo.bindingPath) {
                            propertyValue = {
                                ...
                                targetEntityName: bindingInfo.targetEntityName,
...
exports.getAnnotationFile = function () {
    ...
    var _addProperty = function (currentElement, propertyName, propertyValue, propertyType) {
        if (propertyValue.isSimple || typeof propertyValue === 'string' || !propertyValue.type) {
            if (propertyValue.isNavigationURL) {
                ...
                switch (propertyTypeString) {
                    ...
                    case 'Edm.AnnotationPath' :
                        currentElement.ele('PropertyValue', {
                            Property: propertyName,
                            AnnotationPath: propertyValue.annotationPath || propertyValue.bindingPath
                        });
                        break;
                    default:
            ...
            if (recordInfo) {
                if (recordInfo.isSimple) {
                    ...
                    _.each(recordInfo.annotations, function (subAnnotationValue, subAnnotationName) {
                        var term = odataHelper.findTerm(subAnnotationName);
                        var type = odataHelper.findType(term.Type);
                        if (subAnnotationValue !== null && subAnnotationValue.value != null) {
                            var enumMember = '';
                            if (_.isEmpty(subAnnotationValue.value)) {
                                if (term.FullName === 'com.sap.vocabularies.UI.v1.Importance') {
                                    enumMember = 'com.sap.vocabularies.UI.v1.ImportanceType/High'; // Default value for importance
                                }
                                else {
                                ...
                                enumMember = type.FullType + '/' + subAnnotationValue.value;
                            }
    ...
    _.each(annotations, function (annotationPerEntity, entityName) {
        ...
        _.forIn(annotationPerEntity, function (annotationValue, annotationName) {
            if (annotationName !== 'entityName') {
                ...
                if (annotationValue.isCollection) {
                    ...
                    _.each(annotationValue.properties, function (subPropertyValue, propertyName) {
                        if (subPropertyValue !== null) {
                            _addProperty(mainRecord, propertyName, subPropertyValue, annotationType.properties[propertyName]);
                        }
                    });
                }
            }
    ...
    function setNav(pageInfo, path) {
        ...
        _.forEach(entity.navigationProperties, function (navProp) {
            if (currPath.indexOf(navProp.toEntityId) === -1) {
                var target = builderUtils.getEntityById(navProp.toEntityId);
