    ...
    _.each(req.body, function (value, key) {
        switch (key) {
            case constants.operationCreatePrototype:
                waitForProcessing = that.pageMetadataService.createApp(projectId);
                break;
            case constants.operationCreatePage:
                waitForProcessing = that.pageMetadataService.createPages(projectId, false, value.applicationType, value.pageArray);
                break;
            ...
            case constants.operationModelChange:
                var deletedEntityIds = _.chain(value.operations).filter({
                    type: 'entity',
                    operation: 'delete'
                }).pluck('previous').pluck('_id').value();
