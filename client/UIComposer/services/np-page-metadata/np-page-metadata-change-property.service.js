...
var npPageMetadataChangeProperty = ['npPageMetadataHelper', 'npPageMetadataEvents',
    function (pageMdHelper, pageMdEvents) {
        ...
        var performPropertyChange = function (controlMd, changedProperties, type) {
         ...
         */
        var performPropertyChanges = function (propertyChanges, pageMd, options) {
            var returnObjs = [];

            _.forEach(propertyChanges, function (propertyChange) {
                returnObjs.push(propertyChange.controlMd);
                performPropertyChange(propertyChange.controlMd, propertyChange.properties, propertyChange.propertyType);
            });

            pageMdEvents.broadcast(pageMdEvents.events.controlPropertiesChanged, pageMd, propertyChanges, options);
