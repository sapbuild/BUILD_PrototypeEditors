...
var npPageMetadataDeleteControl = ['$log', 'npPageMetadataHelper', 'npPageMetadataEvents', 'npUiCatalog',
    function ($log, pageMdHelper, pageMdEvents, npUiCatalog) {

        var _isDeletable = function (controlMd) {
            var undeletable = npUiCatalog.isUndeletable(controlMd.catalogControlName, controlMd.catalogId);
            if (undeletable) {
                // this is bad but all other errors are silent here in npPageMetadataDeleteControl...
                $log.error('npPageMetadataDeleteControl: control is not deletable', controlMd);
            }
            return !undeletable;
        };
