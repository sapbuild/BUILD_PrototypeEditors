...
var npTreeModel = ['$rootScope', '$log', 'npUiCatalog', 'npLayoutHelper', 'npGrid', 'npPrototype', 'npPageMetadata', 'npTreeNodeFactory',
    function ($rootScope, $log, npUiCatalog, npLayoutHelper, npGrid, npPrototype, npPageMetadata, nodeFactory) {
        ...
        /**
         * @name invalidateNodes
         * @memberof npTreeModel
         */
        var invalidateNodes = function () {
            _nodes = [];
        };
        ...
        var generateNodes = function (pages, currentPageIndex, gridRoot) {
            var pageNodes = _.map(pages, function (page) {
            ...
            // FIXME this is modifying the gridElement reference in order to make delete possible
            pageNodes[currentPageIndex].data.name = currentPageNode.data.name;
    ...
    // FIX 2581 : we need to reorder aggregation in the same as in floorplans
    if (_isSmart) {
        var sortedGroup = [];
        if (!_.isEmpty(controlMd.groups)) {
            _.forEach(controlMd.groups, function (controlMdGroup) {
                var group = _.find(groups, {name: controlMdGroup.groupId});
                if (!_.isUndefined(group)) {
                    sortedGroup.push(group);
                }
            });
        }
        if (_.isEmpty(sortedGroup)) {
            sortedGroup = groups;
        }
    } else {
        sortedGroup = groups;
    }
    _.forEach(sortedGroup, function (group) {
