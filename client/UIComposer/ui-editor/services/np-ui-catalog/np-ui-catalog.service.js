 ...
 * @module npUiCatalog
 */
var npUiCatalog = ['$resource', '$q', function ($resource, $q) {
    ...
    var getFloorplans = function () {
        // "ui5" will get both "openui5" and "sapui5"
        return catalogAPI.getFloorplans({libraryType: 'ui5'}).$promise.then(flattenFloorplans);
    };

    var flattenFloorplans = function (catalogs) {
        var floorplans = [];
        _.forEach(catalogs, function (catalog) {
            _.each(catalog.floorPlans, function (floorplan) {
                var fp = {
                    text: floorplan.text || floorplan.name,
                    description: floorplan.description,
                    floorplan: floorplan.name,
                    isSmart: floorplan.isSmart,
                    isRootFloorplan: floorplan.isRootFloorplan,
                    catalogName: catalog.catalogName,
                    thumbnail: floorplan.thumbnail,
                    iconClass: floorplan.iconClass,
                    catalogId: catalog._id || catalog.catalogId
                };
                floorplans.push(fp);
            });
        });
    ...
    var getControlPropertiesFromFloorplan = function (customCatalogId, controlName) {
        return getCompatibleCatalogs(customCatalogId)
            .then(function (catalogInfos) {
                var catalogs = _.map(catalogInfos, function (catalogInfo) {
                    return loadedCatalogs[catalogInfo.catalogId];
                });
                var splitName = controlName.split('-');
                var floorPlanName = splitName[0];
                var controlArray = catalogs[0]['floorPlans'][floorPlanName]['controls']['controls'];
                var control = _.find(controlArray, {catalogControlName: controlName});
    ...
    var getCompatibleFloorplans = function (customCatalogId) {
        return getCompatibleCatalogs(customCatalogId).then(function (catalogInfos) {
            var catalogs = _.map(catalogInfos, function (catalogInfo) {
                return loadedCatalogs[catalogInfo.catalogId];
            });
            return flattenFloorplans(catalogs);
        });
    };
    ...
    var _extends = function (control, rootParentName, rootCatalogId) {
        if (!control.additionalMetadata.hasOwnProperty('parent')) {
            return false;
        }
    ...
    var _getReturnObj = function (control, catalogId) {
        return {
            ...
            iconClass: control.additionalMetadata.iconClass
        };
    };

    /**
     * @name getCompatibleCatalogs
     ...
     */
    var getCompatibleCatalogs = function (customCatalogId) {

        var deferred = $q.defer();


        protoTypeCustomCatalogId = customCatalogId;

        var catalogInfo = [];
        // get all compatible catalogs from API
        var response = catalogAPI.getCompatibleCatalogs({catalogId: customCatalogId});
     ...
     * @param {Event} event Event for which to filter the actions
     * @returns {Actions[]|undefined} Array of actions or undefined if catalog is not loaded
     */
    var getActions = function (event) {
        if (_.isEmpty(mainRootCatalog.actions)) {
            return [];
        }

        if (event && _.size(event.possibleActions)) {
            return _.filter(mainRootCatalog.actions, function (action) {
                return _.contains(event.possibleActions, action.actionId);
            });
        }
        return _.filter(mainRootCatalog.actions, 'displayToUser');
    };
    ...
    /**
     * @name isUndeletable
     * @description Retrieve property isUndeletable of the given control
     * @param controlName
     * @param catalogId
     * @returns {boolean}
     */
    var isUndeletable = function (controlName, catalogId) {
        var undeletable = false,
            control = getControlByName(controlName, catalogId) || {};
        if (!_.isEmpty(control.additionalMetadata)) {
            undeletable = !!control.additionalMetadata.isUndeletable;
        }
        return undeletable;
    };
    ...
    /**
     * @name isDataDrivenAggregation
     * @description Retrieve the property isDataDriven for the given aggregation of the given control
     * @param {string} aggregationName name of aggregation
     * @param {string} controlName Name of the control
     * @param {string} catalogId id of catalog of control
     * @returns {boolean} Property isDataDriven of the aggregation
     */
    var isDataDrivenAggregation = function (aggregationName, controlName, catalogId) {
        var aggregations = getControlAggregations(controlName, catalogId);
        if (aggregations[aggregationName]) {
            return aggregations[aggregationName].isDataDriven;
        }
    };
    ...
    var getAggregationsOfContextProperty = function (propertyName, controlName, catalogId) {
        var aggregationNamesOfContextProperty = [];
        if (isContextProperty(propertyName, controlName, catalogId)) {
            var aggregations = getControlAggregations(controlName, catalogId);
            aggregationNamesOfContextProperty = _.pluck(_.where(aggregations, {contextProperty: propertyName}), 'name');
        }
        return aggregationNamesOfContextProperty;
    };
    ...
    /**
     * @name isAdvanced
     * @description Retrieve the property isAdvanced for the given property of the given control
     * @param {string} propertyName name of property for which type is needed
     * @param {string} controlName Name of the control
     * @param {string} catalogId id of catalog of control
     * @returns {boolean} Property isAdvanced
     */
    var isAdvanced = function (propertyName, controlName, catalogId) {
        var properties = getControlProperties(controlName, catalogId);
        if (properties[propertyName]) {
            return !_.isUndefined(properties[propertyName].isAdvanced) ? properties[propertyName].isAdvanced : false;
        }
    };
    ...
    var getPropertyPossibleValues = function (propertyName, controlName, catalogId) {
        var properties = getControlProperties(controlName, catalogId);
        if (properties[propertyName] && properties[propertyName].possibleValues) {
            var result = [];
            //we make sure that possibleValues are all respecting the format {value, displayValue}
            _.each(properties[propertyName].possibleValues, function (possibleValue) {
                if ((typeof possibleValue) !== 'object') {
                    result.push({
                        value: possibleValue,
                        displayValue: possibleValue
                    });
                }
                else {
    ...
    return {
        ...
        getCompatibleCatalogs: getCompatibleCatalogs,
        ...
        getAggregationsOfContextProperty: getAggregationsOfContextProperty,
        getFloorplans: getFloorplans,
        getCompatibleFloorplans: getCompatibleFloorplans,
        getControlPropertiesFromFloorplan: getControlPropertiesFromFloorplan,
        ...
        isDataDrivenAggregation: isDataDrivenAggregation,
