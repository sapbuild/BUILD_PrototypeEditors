    ...
    function (npGrid, npUiCanvasAPI, npCanvasInteractionHelper, $compile, $timeout, npConstants, npKeyboarder, inlineEditHelper, npPageMetadata, npPropertyChangeObserver) {
        return {
            ...
            link: function (scope, element) {
                ...
                //Re-initializes keyboard listeners, inline-editor
                onAfterEnter();
                ...
                function onAfterEnter() {
                    stopListening = inlineEditHelper.startListening(onInlineEditStart);
                    kl1 = npKeyboarder.on(npConstants.keymap.Escape, onEscape.bind(this));
                    kl2 = npKeyboarder.on(npConstants.keymap.Enter, onEnter.bind(this));
                }

                scope.$on('ui-composer/afterEnter', onAfterEnter);
                scope.$on('ui-composer/beforeExit', onBeforeExit);
                scope.$on('$destroy', onBeforeExit);
            }
        };
    }
];
