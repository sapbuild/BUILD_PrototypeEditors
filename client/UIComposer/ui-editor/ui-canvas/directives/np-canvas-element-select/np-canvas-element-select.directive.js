            ...
            link: function (scope) {
                ...
                var showSelectionMenu = function (event) {
                    ...
                    if (elementsAtPosition.length) {
                        ...
                        ml4 = npCanvasInteractionHelper.on(npCanvasInteractionHelper.supportedEvents.dragstart, dismissSelectionMenu);
                        angular.element(document.querySelector('.np-e-canvas-container-js')).on('scroll', dismissSelectionMenu);
                        scope.$apply();
                    }
                };

                var cleanUpInteractionListeners = function (listeners) {
                ...
                var dismissSelectionMenu = function () {
                    ...
                    cleanUpInteractionListeners([ml3, ml4]);
                    angular.element(document.querySelector('.np-e-canvas-container-js')).off('scroll', dismissSelectionMenu);
                    scope.$apply();
                };

                var ml5 = npCanvasInteractionHelper.on(npCanvasInteractionHelper.supportedEvents.contextmenu, showSelectionMenu, true);

                /***********/
                /* Cleanup */
                /***********/
                scope.$on('$destroy', function cleanup() {
                    cleanUpInteractionListeners([ml1, ml2, ml3, ml4, ml5]);
                });
            }
        };
    }
