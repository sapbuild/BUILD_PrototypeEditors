    ...
    function ($rootScope, $resource, $q, $timeout, dataModelService, npConstants, pageMdHelper, npUiCatalog) {
        ...
        var oDataUrl = '/api/projects/',
            ...
            actions = {
                getData: {
                    method: 'GET',
                    url: oDataUrl + ':projectId/prototype/artifact/models/sampleData.json',
                    params: {
                        projectId: '@projectId'
                    }
                }
        ...
        var refreshModel = function () {
            var projectId = that.dataModel ? that.dataModel.projectId : undefined;
        ...
        var initEntities = function (projectId) {
            ...
            that.mockDataPromise = undefined;
        ...
        var getPathsCompatibleWithParentGroup = function (controlMd, entityName, mainEntity) {
            ...
            if (parentCtrl) {
                ...
                if (parentGroup && npUiCatalog.isMultipleAggregation(parentGroup.groupId, parentCtrl.catalogControlName, parentCtrl.catalogId)) {
                    var bindingInfo = getBindingContextInfoFromMd(parentCtrl, mainEntity);
                    if (bindingInfo.isValid) {
                        compatiblePaths = getPathsCompatibleWithEntity(bindingInfo.bindingContext, entityName);
                    }
                }
            }
        ...
        var isInvalidBinding = function (bindingMd, mainEntityId) {
            if (!hasEntities()) {
                return false;
            }
            var mainEntity = _getEntityFromId(mainEntityId),
                path = getPath(bindingMd);

            return !mainEntity || !path || path.indexOf('/' + mainEntity.nameSet) !== 0;
        };
        ...
        var getEntitiesAndProperties = function () {
            if (hasEntities()) {
                var entities = _.map(that.dataModel.entities, function (entity) {
                    var entityInfo = {
                        _id: entity._id,
                        name: entity.name
                    };
                    entityInfo.properties = _.chain(entity.properties)
                        .filter(function (property) {
                            return property.isForeignKey === false;
                        })
        ...
        var _getMockDataForEntity = function (entityId) {
            var mockDataPromise = that.mockDataPromise;
            var currentEntity = _getEntityFromId(entityId),
                keyProperties = _.filter(currentEntity.properties, {isKey: true});
            if (!mockDataPromise) {
                var payload = {projectId: that.projectId};
                mockDataPromise = mockDataAPI.getData(payload).$promise;
                that.mockDataPromise = mockDataPromise;
            }
            return mockDataPromise.then(function (mockData) {
                _.forEach(mockData, function (entityData, nameSet) {
                    _.forEach(entityData, function (entity) {
                        var paths = _.map(keyProperties, function (keyProperty) {
                            return entity[keyProperty.name];
                        });
                        entity.path = '/' + nameSet + '(\'' + paths.join('') + '\')';
                    });
                });
                return mockData[currentEntity.nameSet];
            });
        };
        ...
        var _calculateGroupBinding = function (groupMd, parentControlMd) {
            var binding = null;
            var contextPropertyName = null;
            if (parentControlMd) {
                // Check if the group have a context property (property for smart templates that have the binding for the group)
                contextPropertyName = npUiCatalog.getAggregationContextProperty(groupMd.groupId, parentControlMd.catalogControlName, parentControlMd.catalogId);
            ...
            return {binding: binding, contextPropertyName: contextPropertyName};
        };
        ...
        var getBindingContextInfoFromMd = function (controlMd, mainEntityId) {
            var binding = {isRelative: true, paths: []},
                ...
                nextStep = function () {
                    ...
                    var containingGroupBindingInfo = _calculateGroupBinding(containingGroupMd, parentControlMd);
                    containingGroupBinding = containingGroupBindingInfo.binding;
                    contextPropertyName = containingGroupBindingInfo.contextPropertyName;
            ...
            while (controlTargetMd && ((isParentGroupBound && binding.isRelative) || !isParentGroupBound)) {
                ...
                else if (contextPropertyName) {
                    // There is no binding for the group but it has a context property
                    // The binding context is thus invalid
                    isValid = false;
                }
                controlTargetMd = (controlTargetMd && controlTargetMd.getParentMd) ? controlTargetMd.getParentMd() : undefined;
                nextStep();
            }
            ...
            if (!isValid || _.isEmpty(binding) || _.isEmpty(binding.paths)) {
                binding = undefined;
            }
            return {bindingContext: binding, isValid: isValid};
        };

        var getGroupPathsFromMd = function (groupMd, controlMd, mainEntityId) {
            if (npUiCatalog.isDataDrivenAggregation(groupMd.groupId, controlMd.catalogControlName, controlMd.catalogId)) {
                var bindingContextInfo = {isValid: true};
                if (controlMd) {
                    bindingContextInfo = getBindingContextInfoFromMd(controlMd, mainEntityId);
                }
                return bindingContextInfo.isValid ? getGroupPaths(bindingContextInfo.bindingContext) : undefined;
            }
        };

        var getPropertyPathsFromMd = function (propertyMd, controlMd, mainEntityId, isSmartApp) {
            var possiblePaths, bindingContext;
            if (npUiCatalog.isDataDriven(propertyMd.name, controlMd.catalogControlName, controlMd.catalogId)) {
                var bindingContextInfo = getBindingContextInfoFromMd(controlMd, mainEntityId);
                if (bindingContextInfo.isValid) {
                    bindingContext = bindingContextInfo.bindingContext;
        ...
        var getContextEntity = function (controlMd, mainEntity) {
            var bindingContextInfo = getBindingContextInfoFromMd(controlMd, mainEntity);
            if (bindingContextInfo.isValid && bindingContextInfo.bindingContext) {
                return getEntityFromBinding(bindingContextInfo.bindingContext);
            }
        };
        ...
        var getFirstBindableAncestorAndGroup = function (controlMd, oneLevelOnly) {
            var parentControlMd = controlMd;
            var groupId;
            var groupMd;
            var canBindGroup = false;
            while (parentControlMd && !canBindGroup) {
                groupId = parentControlMd.parentGroupId;
                parentControlMd = parentControlMd.getParentMd();
                groupMd = parentControlMd ? _.find(parentControlMd.groups, {groupId: groupId}) : undefined;
                canBindGroup = groupMd ? npUiCatalog.isDataDrivenAggregation(groupMd.groupId, parentControlMd.catalogControlName, parentControlMd.catalogId) : undefined;
                if (oneLevelOnly) {
                    // Search only in the parent
                    break;
                }
            }
            if (canBindGroup && parentControlMd) {
                return {ancestorControlMd: parentControlMd, ancestorGroupMd: groupMd};
            }
        };

        var getUsedPropertyIdsByEntityId = function (pageMd) {
            var usedModelProperties = {};
            if (pageMd.mainEntity) {
                usedModelProperties[pageMd.mainEntity] = {properties:{}};
            }
            var fillUsedFromBindingFn = function (binding) {
                if (!_.isEmpty(binding) && !_.isEmpty(binding.paths)) {
                    var lastPathPart = _.last(binding.paths);
                    if (!lastPathPart.propertyId) {
                        usedModelProperties[lastPathPart.entityId] = usedModelProperties[lastPathPart.entityId] || {};
                    }
                    else if (!_isNavigationProperty(lastPathPart.entityId, lastPathPart.propertyId)) {
                        usedModelProperties[lastPathPart.entityId] = usedModelProperties[lastPathPart.entityId] || {};
                        usedModelProperties[lastPathPart.entityId][lastPathPart.propertyId] = true;
                    }
                    else {
                        var navProperty = _getPropertyFromId(lastPathPart.entityId, lastPathPart.propertyId);
                        if (navProperty) {
                            usedModelProperties[navProperty.toEntityId] = usedModelProperties[navProperty.toEntityId] || {};
                        }
                    }
                }
            ...
            _.each(pageMd.controls, function (control) {
                _.each(control.properties, function (property) {
                    fillUsedFromBindingFn(property.binding);
                });
                _.each(control.groups, function (group) {
                    fillUsedFromBindingFn(group.binding);
                });
            });
            return usedModelProperties;
        };
        ...
        return {
            ...
            getFirstBindableAncestorAndGroup: getFirstBindableAncestorAndGroup,
            getUsedPropertyIdsByEntityId: getUsedPropertyIdsByEntityId,
