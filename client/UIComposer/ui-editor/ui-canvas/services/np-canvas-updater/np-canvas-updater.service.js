...
var npCanvasUpdater = ['$q', 'npUiCanvasAPI', 'npCanvasEvents', 'npPageMetadataEvents', 'npPageMetadataHelper',
    function ($q, npUiCanvasAPI, npCanvasEvents, pageMdEvents, pageMdHelper) {
        ...
        function controlsBindingChangedListener(event, pageMd, bindingDefs) {
            npUiCanvasAPI.navTo(pageMd)
                .then(function () {
                    _.forEach(bindingDefs, updateCanvasBinding);
                    var changedControls = _.pluck(bindingDefs, 'controlMd');
                    broadcastWhenReady(changedControls);
                });
        }
         ...
         * @param {PageMd} pageMd
         */
        function updateCanvasBinding(bindingDef) {
            if (!pageMdHelper.isBound(bindingDef.groupMd)) {
                npUiCanvasAPI.unbindControlGroup(bindingDef.controlMd, bindingDef.groupMd);
            }
            else {
                npUiCanvasAPI.bindControlGroup(bindingDef.controlMd, bindingDef.groupMd);
            }
        }
    }
];
