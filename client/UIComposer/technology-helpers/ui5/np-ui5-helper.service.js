...
var npUi5Helper = ['$q', '$log', '$timeout', 'npLayoutHelper',
    function ($q, $log, $timeout, npLayoutHelper) {
        ...
        var wait = function (seconds) {
            var deferred = $q.defer();
            setTimeout(function () {
                $log.info('timeout expired', seconds);
                deferred.resolve();
            }, seconds * 1000);
        ...
        var coreReady = function () {
            var deferred = $q.defer();
            var core = win && win.sap ? win.sap.ui.getCore() : undefined;
            if (!core) {
                deferred.reject('cannot find core');
            }
            else {
                core.attachInit(deferred.resolve);
                wait(60).then(deferred.reject);
            }
        ...
        var waitForBinding = function (control, bindingName) {
            ...
            if (_.isEmpty(bindingInfo)) {
                $log.info('waitForBinding: no binding found. not waiting...', control, bindingName);
                return $q.when();
            }
            // first wait to receive data, then for eventual rendering
            return waitForDataReceived(control, bindingInfo.binding).then(waitForRendering);
        };
        ...
        var getBindingInfo = function (ctrl, name) {
            ...
            if (!isMashupControl(ctrl)) {
                ...
                bindingInfo = {};
                if (!_.isEmpty(info)) {
                    bindingInfo.path = info.path || ctrl.getBindingPath(name);
                    bindingInfo.binding = info.binding;
                    bindingInfo.template = info.template;
                    bindingInfo.expandPaths = _.isEmpty(info.parameters) ? null : info.parameters.expand;
                }
            }
            return bindingInfo;
        };
        ...
        var getCurrentViewBindingInfo = function () {
            ...
            if (view) {
                ...
                if (!_.isEmpty(info)) {
                    bindingInfo.path = info.path || info.getPath();
                    bindingInfo.binding = info.binding;
                    bindingInfo.template = info.template;
                    bindingInfo.expandPaths = _.isEmpty(info.parameters) ? null : info.parameters.expand;
                }
        ...
        var bindAggregation = function (control, aggregationName, bindingInfo) {
            unbindAggregation(control, aggregationName, !bindingInfo);
            if (bindingInfo && bindingInfo.path && bindingInfo.template) {
                var ui5BindingInfo = {parameters: {}};
                ui5BindingInfo.path = bindingInfo.path;
                ui5BindingInfo.binding = bindingInfo.binding;
                ui5BindingInfo.template = bindingInfo.template;
                ui5BindingInfo.parameters.expand = bindingInfo.expandPaths || '';

                control.bindAggregation(aggregationName, ui5BindingInfo);
            }
        };

        var unbindAggregation = function (control, aggregationName, hideBusyCursor) {
            if (control.isBound(aggregationName)) {
                control.unbindAggregation(aggregationName);
                if (hideBusyCursor && control.getEnableBusyIndicator && control.setEnableBusyIndicator) {
                    // Workaround to remove the busy cursor when we bind an agregation and before data is loaded, we unbind the aggregation, the busy cursor is not hidden on unbindAggregation of the control
                    var enableBusyCursor = control.getEnableBusyIndicator();
                    control.setEnableBusyIndicator(false);
                    control.setEnableBusyIndicator(enableBusyCursor);
                }
            }
        };
         ...
         * @param isSmartApp check if this is a smart app or not
         * @returns {string} The current view's base name.
         */
        var getCurrentViewName = function (isSmartApp) {
            var currentView = getCurrentView();
            var currentViewName;
            if (isSmartApp && currentView) {
                var smartConfiguration = getSmartConfiguration();
                if (smartConfiguration && smartConfiguration.pages) {
                    var entitySet = currentView.getParent().getEntitySet();
                    var componentName = getCurrentView().getParent().getMetadata().getName();
                    componentName = componentName.slice(0, _.lastIndexOf(componentName, '.'));

                    var firstPage = smartConfiguration.pages[0];
                    if (firstPage.entitySet === entitySet && firstPage.component === componentName) {
                        currentViewName = firstPage.pageName;
                    }
                    else {
                        var targetPage = _.find(smartConfiguration.pages[0].pages, function (config) {
                            return config.entitySet === entitySet && config.component === componentName;
                        });
                        currentViewName = targetPage.pageName;
                    }
                }
                else {
                    currentViewName = currentView.getViewName();
                    currentViewName = currentViewName.slice(_.lastIndexOf(currentViewName, '.') + 1);
                }
            }
            else if (currentView) {
                currentViewName = currentView.getViewName();
        ...
        var navTo = function (routeName) {
            ...
            if (win) {
                var smartConfiguration = getSmartConfiguration();
                var currentView = getCurrentView();
                var currentViewName = getCurrentViewName((smartConfiguration !== null && smartConfiguration.pages !== null));
                // if already in the page, resolve
                if (currentViewName && currentViewName.endsWith(routeName)) {
                    deferred.resolve();
                }
                // else wait for navigation
                    ...
                    wait(30).then(function () {
                        app[getAttachMethod(false)](fnAfterNavigate);
                        deferred.reject('navTo: takes longer than 30s, couldn\'t nav to route ' + routeName);
                    });
        ...
        var getClass = function (className) {
            try {
                ...
                return win.jQuery.sap.getObject(className);
            }
            catch (e) {
         ...
         * @name reloadMockData
         * @memberof npUi5Helper
         * @description reload the mock data
         */
        var reloadMockData = function () {
            var mockServer = getMockServer();
            mockServer._refreshData();
            if (typeof mockServer.calculated === 'function') {
                mockServer.calculated();
            }
        };
        ...
        var refreshPageFromAnnotation = function () {
            if (that.isRefreshingPage) {
                that.needToRefresh = true;
            }
            else {
                that.isRefreshingPage = true;
                var view = getCurrentView();
                    ...
                    var _reloadView = function () {
                        viewComponent.runAsOwner(function () {
                            try {
                                ...
                                $log.error('_reloadView() failed', err);
                            }
                            finally {
                                _.delay(function () {
                                    that.isRefreshingPage = false;
                                    if (that.needToRefresh) {
                                        that.needToRefresh = false;
                                        refreshPageFromAnnotation();
                                    }
                                }, 500);
                            }
                        });
