...
function modelEditorService($log, $q, $window, $upload, $rootScope, Model, Entity, Property, Navigation, Group) {
    ...
    this.selectAnotherEntity = function (removedEntityId) {
        if (this.selectedEntity && this.selectedEntity._id === removedEntityId) {
            if (this.model && this.model.entities && this.model.entities.length > 0) {
                var anotherEntity;
                this.model.entities.some(function (entity) {
                    if (entity._id !== removedEntityId) {
                        anotherEntity = entity;
                        return true;
                    }
                });
                this.setSelectedEntity(anotherEntity);
            }
            else {
                this.setSelectedEntity(null);
            }
    ...
    this.removeEntity = function (entityToRemoveId) {
        // if parameter is undefined, remove selected entity
        if (!entityToRemoveId) {
            entityToRemoveId = self.selectedEntity._id;
        }
        // if the entity to remove is the current selection, change selection
        self.selectAnotherEntity(entityToRemoveId);

        // update the model right now, without waiting for server response
        _.remove(self.model.entities, {_id: entityToRemoveId});
        angular.forEach(self.model.entities, function (entity) {
            _.remove(entity.navigationProperties, {toEntityId: entityToRemoveId});
        });
        $rootScope.$broadcast('ModelEditorService.modelChanged', self.model);

        return self._pushInWaitingQueue(self._doRemoveEntity, [entityToRemoveId]);
    };

    this._doRemoveEntity = function (id) {
        var params = {
            id: id,
            modelId: self.model.projectId
        };
    ...
    this.addProperty = function (propertyData) {
        return self._pushInWaitingQueue(self._doAddProperty, [propertyData, this.selectedEntity._id]);
    };

    this._doAddProperty = function (propertyData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.updateProperty = function (propertyData) {
        return self._pushInWaitingQueue(self._doUpdateProperty, [propertyData, this.selectedEntity._id]);
    };

    this._doUpdateProperty = function (propertyData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.removeProperty = function (propertyData) {
        return self._pushInWaitingQueue(self._doRemoveProperty, [propertyData, this.selectedEntity._id]);
    };

    this._doRemoveProperty = function (propertyData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.addNavigation = function (navigationData) {
        return self._pushInWaitingQueue(self._doAddNavigation, [navigationData, this.selectedEntity._id]);
    };

    this._doAddNavigation = function (navigationData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.updateNavigation = function (navigationData) {
        return self._pushInWaitingQueue(self._doUpdateNavigation, [navigationData, this.selectedEntity._id]);
    };

    this._doUpdateNavigation = function (navigationData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.addGroup = function (groupData) {
        return self._pushInWaitingQueue(self._doAddGroup, [groupData, this.selectedEntity._id]);
    };

    this._doAddGroup = function (groupData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
    ...
    this.updateGroup = function (groupData) {
        return self._pushInWaitingQueue(self._doUpdateGroup, [groupData, this.selectedEntity._id]);
    };

    this._doUpdateGroup = function (groupData, entityId) {
        var params = {
            modelId: self.model.projectId,
            entityId: entityId
        };
