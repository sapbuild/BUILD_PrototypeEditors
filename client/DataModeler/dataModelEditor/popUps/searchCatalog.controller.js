...
function SearchCatalog($scope, $timeout, ModelEditorService, Catalog) {
    ...
    $scope.$on('dialog-open', function (event, args) {
        if (args === 'search-entity-panel') {
            $timeout(function () {
                var input = document.getElementById('dm-catalog-search-input');
                if (input) {
                    input.focus();
                }
            });
...
module.exports = ['$scope', '$timeout', 'dm.ModelEditorService', 'dm.Catalog', SearchCatalog];
