...
(function () {
    ...
    function FormulaEditorController($scope, $rootScope, $timeout, ModelEditorService) {
        ...
        vm.placementIsValid = function (placedElementId, inputArray) {
            var result = true;
            if (inputArray[0].id === placedElementId) {
                return false;
            }
            for (var k = 0; k < inputArray.length; k++) {
                if (inputArray[k].id === placedElementId && inputArray[k].type !== 'operator') {
                    result = false;
                    break;
                }
                else {
                    var parent = vm.getParent(placedElementId, ModelEditorService.model.editedProperty.calculated.calculation);
                    if (parent.id) {
                        result = vm.placementIsValid(placedElementId, parent);
                        if (!result) {
                            break;
                        }
                    }
        ...
        vm.addElement = function (inputArray, addedStuff, place, id, container, addAll, droppedInRight) {
            var addedObject;
            var length;
            var remainingStuff = [];
            var oResult;
            ...
            if (typeof addedStuff === 'string') {
                ...
                if (addedStuff.left || addedStuff.right) {
                    leftLength += addedStuff.left.length;
                    addedStuff.right = [];
                    addedStuff.left = [];
                }
            ...
            if (id === -1 && container === '') {
                ...
                formulaObj = vm.getElement(id, ModelEditorService.model.editedProperty.calculated.calculation)[container];
            }
            else {
                if (typeof place === 'string') {
                    place = parseInt(place, 10) + (droppedInRight ? leftLength : 0);
                }
            ...
            if (Array.isArray(formulaObj)) {
                if (formulaObj.length === 1 && formulaObj[0].hasOwnProperty('right') && !addedObject.hasOwnProperty('left')) {
                    vm.addElement(formulaObj, addedObject, place, formulaObj[0].id, 'right', addAll, droppedInRight);
                }
                else {
        ...
        $scope.defineDrag = function (scope, element, attrs) {
            ...
            element.bind('dragstart', function (evt) {
                evt.stopPropagation();
                vm.dropElement = false;
                vm.dropZoneWidth = ((new RegExp('(^|\\s)(dmfe-operatorHandle)(\\s|$)')).test(evt.toElement.className) ? evt.toElement.children[0].children[0].offsetWidth : evt.toElement.offsetWidth) + 'px';
                ...
                if (vm.endDrag) {
                    ...
                    if (evt.toElement.nodeName !== '#text' && (new RegExp('(^|\\s)(dmfe-elementButton)|(dmfe-operatorHandle)|(dmfe-inlineElement)|(dmfe-condition)(\\s|$)')).test(evt.toElement.className)) {
                        if (vm.dropElement && evt.toElement.dataset.element !== 'setElement') {
                            vm.addElement(ModelEditorService.model.editedProperty.calculated.calculation, evt.toElement.dataset.element, vm.droppedIntoElement.dataset.added, parseInt(vm.droppedIntoElement.dataset.eid, 10), vm.droppedIntoElement.dataset.container, false);
                        }
                        else if (vm.dropElement && (parseInt(vm.droppedIntoElement.dataset.eid, 10) === -1 || vm.placementIsValid(parseInt(evt.toElement.dataset.eid, 10), [vm.getElement(parseInt(vm.droppedIntoElement.dataset.eid, 10), ModelEditorService.model.editedProperty.calculated.calculation)]))) {
                            ...
                            var droppedIntoRight = vm.droppedIntoElement.parentElement.dataset.operand === 'right' && parseInt(vm.droppedIntoElement.parentElement.parentElement.previousElementSibling.dataset.eid, 10) === subElement.id;
                            vm.addElement(ModelEditorService.model.editedProperty.calculated.calculation, subElement, vm.droppedIntoElement.dataset.added, parseInt(vm.droppedIntoElement.dataset.eid, 10), vm.droppedIntoElement.dataset.container, all, droppedIntoRight);
                        }
        ...
        $scope.defineDrop = function (scope, element, attrs) {
            ...
            element.bind('dragover', function (evt) {
                ...
                else if ((new RegExp('(^|\\s)(dmfe-addElement)|(dmfe-operand)|(dmfe-operatorHandle)|(dmfe-valueDiv)|(dmfe-propertyDiv)(\\s|$)')).test(element[0].className)) {
                ...
                else if ((!vm.deletableElement || element[0].dataset.eid !== vm.deletableElement.dataset.eid) && element[0].dataset.element === 'setElement') {
